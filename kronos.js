...
let process_name = "retroarch.exe"
let process = memoryjs.openProcess(process_name)
//console.log(process)

// this is a consistens start of a lowram. If memory inspector is present, this will find it instead of the actual data. It's fine, though (unless you scroll said inspector)
...
// the above thing worked SOME times but then didn't other. So there's more fidgeting here and below when I do the  "+ 984 -13304" crap
...
console.log(sig_work_ram_high_length)


// still no idea where the high ram relative to the low ram
// for all I know it can be placed *lower* than the low (at least in RaLibRetro)
// for example: sig_work_ram_low - sig_work_ram_high = 0x216000 (2187264) 

// this searches relatively fast
...
// one needs to do this in CheatEngine, but not in memoryjs. No idea why
...
// this searches *incredibly* slow in RALibRetro, maybe the start of this pattern is really repetitive?
...
console.log("Please check if these are correct:")
console.log("MapID", pat, '0x'+(pat).toString(16))
console.log("Arthur's HP", pat2, '0x'+(pat2).toString(16))
